...
CCC.STATIC.UTIL = {
  ...
  convertValueToDisplay(symbol, value, filterNumberFunctionAngularJS, type, fullNumbers) {
    ...
    if (type === 'short' || type === '8decimals') {
      if (valueAbs > 10000000000) {
        valueAbs /= 1000000000;
        return [prefix, filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers), ' B'].join('');
      }
      if (valueAbs > 10000000) {
        valueAbs /= 1000000;
        return [prefix, filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers), ' M'].join('');
      }
      if (valueAbs > 10000) {
        valueAbs /= 1000;
        return [prefix, filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers), ' K'].join('');
      }
      if (type === '8decimals' && valueAbs >= 100) {
        return prefix + filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnBigNumbers);
      }
      if (valueAbs >= 1) {
        return [prefix, filterNumberFunctionAngularJS(valueSign * valueAbs, decimalsOnNormalNumbers)].join('');
      }
  ...
  dataUnpack(data) {
    ...
    if (currentPrice[pair].LASTTRADEID) {
      currentPrice[pair].LASTTRADEID = parseInt(currentPrice[pair].LASTTRADEID, 0).toFixed(0);
    }

    const CHANGE24HOUR = CCC.convertValueToDisplay(tsym, (currentPrice[pair].PRICE -
...
CCC.convertValueToDisplay = (symbol, value, type, fullNumbers) => {
  ...
  if (val < 0.00001000 && val >= 0.00000100 && decimalsOnSmallNumbers > 3) {
    decimalsOnSmallNumbers = 3;
  }
  if (val < 0.00000100 && val >= 0.00000010 && decimalsOnSmallNumbers > 2) {
    decimalsOnSmallNumbers = 2;
  }
  if (val < 0.00000010 && val >= 0.00000001 && decimalsOnSmallNumbers > 1) {
    decimalsOnSmallNumbers = 1;
  }
  ...
  if (type === 'short') {
    if (valueAbs > 10000000000) {
      valueAbs /= 1000000000;
      return [prefix, CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers), ' B'].join('');
    }
    if (valueAbs > 10000000) {
      valueAbs /= 1000000;
      return [prefix, CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers), ' M'].join('');
    }
    if (valueAbs > 10000) {
      valueAbs /= 1000;
      return [prefix, CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnBigNumbers), ' K'].join('');
    }
    if (valueAbs >= 1) {
      return [prefix + CCC.filterNumberFunctionPolyfill(valueSign * valueAbs, decimalsOnNormalNumbers)].join('');
    }
    return prefix + (valueSign * valueAbs).toPrecision(decimalsOnSmallNumbers);
  }
